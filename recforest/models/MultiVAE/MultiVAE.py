...
class MultiVAE(Model):
    def __init__(self, dataset: Dataset, config: dict) -> None:
        super(MultiVAE, self).__init__(dataset, config)
        self.user_item_matrix = torch.tensor(self.dataset.user_item_matrix.todense(), dtype=torch.float32)

        self.dropout = torch.nn.Dropout(p=0.5)
        self.encoder_dim = [self.dataset.item_cnt] + self.config['encoder_dim']
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        input += torch.rand_like(input) * self.config['noise']
